...
class TableCanvas(Canvas):
    ...
    def mouse_wheel(self, event):
        ...
        # TODO how about for Mac OS ,  seems this event.delta does not work
    ...
    def redrawVisible(self, event=None, callback=None):
        ...
        self.rowrange = list(range(0,int(self.rows)))
    ...
    def setSelectedCells(self, startrow, endrow, startcol, endcol):
        ...
        for c in range(int(startcol), int(endcol)):
    ...
    def select_All(self):
        ...
        self.multiplerowlist = list(range(int(self.startrow),int(self.endrow)))
        self.drawMultipleRows(self.multiplerowlist)
        self.startcol = 0
        self.endcol = self.cols
        self.multiplecollist = list(range(int(self.startcol), int(self.endcol)))
        self.drawMultipleCells()
        return
    ...
    def handle_mouse_drag(self, event):
        ...
        if colover > self.cols or self.startcol > self.cols:
            ...
            if self.endcol < self.startcol:
                self.multiplecollist=list(range(int(self.endcol), int(self.startcol+1)))
            else:
                self.multiplecollist=list(range(int(self.startcol), int(self.endcol+1)))
            #print self.multiplecollist
        #draw the selected rows
        if self.endrow != self.startrow:
            if self.endrow < self.startrow:
                self.multiplerowlist=list(range(int(self.endrow), int(self.startrow+1)))
            else:
                self.multiplerowlist=list(range(int(self.startrow), int(self.endrow+1)))
            self.drawMultipleRows(self.multiplerowlist)
            self.tablerowheader.drawSelectedRows(self.multiplerowlist)
    ...
    def setcellColor(self, rows, cols=None, newColor=None, key=None, redraw=True):
        ...
        if self.allrows == True:
            ...
            rows = list(range(0,int(self.rows)))
        if cols == None:
            cols = list(range(self.cols))
        for col in cols:
            for row in rows:
    ...
    def drawTooltip(self, row, col):
        ...
        if type(text) is float or type is int:
            text=str(text)
        if text == type(None) or text == '' or len(str(text))<=3:
            return
...
class ColumnHeader(Canvas):
    ...
    def handle_left_release(self,event):
        ...
        self.delete('resizesymbol')
        #move column
        if hasattr(self, 'draggedcol') and self.draggedcol != None and self.table.currentcol != self.draggedcol:
    ...
    def handle_left_shift_click(self, event):
        ...
        if colclicked > currcol:
            self.table.multiplecollist = list(range(int(currcol), int(colclicked+1)))
        elif colclicked < currcol:
            self.table.multiplecollist = list(range(int(colclicked), int(currcol+1)))
        else:
            return
...
class RowHeader(Canvas):
    ...
    def handle_mouse_drag(self, event):
        ...
        if self.endrow != self.startrow:
            if self.endrow < self.startrow:
                rowlist=list(range(int(self.endrow), int(self.startrow+1)))
            else:
                rowlist=list(range(int(self.startrow), int(self.endrow+1)))
